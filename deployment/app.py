"""
Energy Poverty Prediction Dashboard — CAP5771 Milestone 3
Author: npydipati

All 9 static views and 3 interactive widgets are taken directly from
data_visualization_static.ipynb and wrapped in Streamlit pages.

Data files needed (produced by data_modeling.ipynb):
  data/processed/analysis_ready.csv
  data/processed/test_predictions.csv       (cols: country, year, electricity_access,
                                             risk_category, pred_electricity_access,
                                             pred_risk_category)
  data/processed/feature_importance.csv     (cols: feature, importance, model)
  data/processed/forecast_2024_2027.csv     (cols: country, year, pred_electricity_access,
                                             pred_risk_category)
"""
...
RISK_COLORS = ["#D32F2F", "#FFA000", "#388E3C"]


# ─── Data loaders ────────────────────────────────────────────────────────────
def _find(filename):
    """Search for a file in a few common locations."""
    candidates = [
        filename,
...
# ─── Fallback model training (if CSVs from modeling notebook are missing) ───
@st.cache_resource(show_spinner="Output CSVs not found — training models from scratch …")
def train_fallback(df):
    """Train RF models and produce the three output DataFrames."""
    le = LabelEncoder()
    le.fit(RISK_ORDER)
...
# ─── Sidebar ─────────────────────────────────────────────────────────────────
def sidebar():
    st.sidebar.title("⚡ Energy Poverty")
    st.sidebar.caption("CAP5771 · Milestone 3 · npydipati")
...
def page_overview(df):
    ...
    st.markdown(
        "**CAP5771 Milestone 3 · npydipati** — All 9 views and 3 interactive widgets are "
        "reproduced directly from `data_visualization_static.ipynb`. "
...
def page_views_1_3(df):
    ...
    fig.suptitle("View 1 — Global Electricity Access (1990–2023)", fontsize=13, fontweight="bold")

    global_avg = df.groupby("year")["electricity_access"].mean()
    axes[0].plot(global_avg.index, global_avg.values, color="steelblue", linewidth=2.5)
    axes[0].fill_between(global_avg.index, global_avg.values, alpha=0.15, color="steelblue")
    axes[0].axhline(90, color="green", linestyle="--", linewidth=1.5, label="90% — Minimal threshold")
    axes[0].axhline(50, color="red",   linestyle="--", linewidth=1.5, label="50% — Severe threshold")
    ...
    axes[1].axvline(latest_df.mean(), color="darkred", linestyle="--", linewidth=2,
                    label=f"Mean = {latest_df.mean():.1f}%")
    axes[1].axvline(50, color="red",   linestyle=":", linewidth=1.5, label="50% boundary")
    axes[1].axvline(90, color="green", linestyle=":", linewidth=1.5, label="90% boundary")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle("View 2 — Risk Category Distribution", fontsize=13, fontweight="bold")

    counts = df["risk_category"].value_counts()
    vals   = [counts.get(c, 0) for c in RISK_ORDER]
    bars   = axes[0].bar(RISK_ORDER, vals, color=RISK_COLORS, edgecolor="white", width=0.5)
    axes[0].set_title("Overall Risk Category Counts (1990–2023)")
    axes[0].set_ylabel("Number of Country-Year Rows"); axes[0].grid(True, axis="y", alpha=0.3)
    ...
    axes[1].axvline(2017, color="black", linestyle="--", linewidth=1, label="Train/Val split (2017)")
    axes[1].axvline(2020, color="grey",  linestyle="--", linewidth=1, label="Val/Test split (2020)")
    ...
    st.caption("Left: top 10 most improved | Right: 15 countries with lowest access today. Dark red bars < 30 %.")

    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle("View 3 — Electricity Access Progress (1990–2023)", fontsize=13, fontweight="bold")
    ...
    colors_sev  = ["#D32F2F" if v < 30 else "#FF7043" for v in severe_now["electricity_access"]]
    axes[1].barh(severe_now["country"], severe_now["electricity_access"],
                 color=colors_sev, edgecolor="white")
    axes[1].axvline(50, color="black", linestyle="--", linewidth=1.5, label="50% Severe threshold")
...
def page_views_4_6(df, feat_imp):
    ...
    fig.suptitle("View 4 — Feature Importance — Best Models", fontsize=13, fontweight="bold")
    ...
    for ax, imp, title in zip(
        ...
        colors_bar = ["#1565C0" if v == imp["importance"].max() else "#90CAF9"
                      for v in imp["importance"]]
        ax.barh(imp["feature"], imp["importance"], color=colors_bar, edgecolor="white")
    ...
    ax.set_xlabel("Pearson Correlation"); ax.grid(True, axis="x", alpha=0.3)
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle("View 6 — Model Performance Comparison (Validation Set)", fontsize=13, fontweight="bold")
    ...
    bars = axes[0].bar(cls_models, cls_scores, color=cls_colors, edgecolor="white", width=0.5)
    ...
    axes[0].grid(True, axis="y", alpha=0.3); axes[0].tick_params(axis="x", labelsize=9)
    ...
    bars2 = axes[1].bar(reg_models, reg_scores, color=reg_colors, edgecolor="white", width=0.5)
    axes[1].set_ylim(0.88, 0.930); axes[1].set_ylabel("Validation R²")
    axes[1].set_title("Regression — Validation R²\nBest: RF Improved (0.9178)")
    axes[1].grid(True, axis="y", alpha=0.3); axes[1].tick_params(axis="x", labelsize=9)
...
def page_views_7_8(df, test_df):
    ...
    fig.suptitle("View 7 — Test Set Performance (2021–2023)", fontsize=13, fontweight="bold")
    ...
    st.caption("Each dot = one country in that reference year. Log₁₀ GDP on x-axis.")
    ...
    ax.axhline(90, color="green", linestyle="--", linewidth=1, alpha=0.7, label="90% Minimal boundary")
    ax.axhline(50, color="red",   linestyle="--", linewidth=1, alpha=0.7, label="50% Severe boundary")
    ax.set_xlabel("Log₁₀ GDP per Capita (USD)"); ax.set_ylabel("Electricity Access (%)")
    ax.legend(title="Year", fontsize=9); ax.grid(True, alpha=0.3)
    plt.tight_layout(); st.pyplot(fig); plt.close()
...
""")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle("View 9 — 2024–2027 Energy Poverty Forecast", fontsize=13, fontweight="bold")
    ...
    for i, (cat, col) in enumerate(zip(RISK_ORDER, RISK_COLORS)):
        ...
        axes[0].bar(x + i * width, vals, width, label=cat, color=col, edgecolor="white", alpha=0.85)
    ...
    axes[1].plot(fore_avg.index, fore_avg.values,
                 color="coral", linewidth=2, linestyle="--", marker="o", label="Forecast (predicted)")
    axes[1].axvline(2023, color="black", linestyle="--", linewidth=1.5, label="Forecast start (2023)")
...
def page_widget_a(df, test_df, forecast_df):
    ...
    fig.suptitle(f"{c} — Electricity Access & GDP per Capita", fontsize=12, fontweight="bold")

    # Left: historical + test + forecast (exact Widget A logic from notebook)
    axes[0].plot(sub["year"], sub["electricity_access"], "o-", color="steelblue",
                 label="Actual", linewidth=2)
    if len(test_sub) > 0:
        axes[0].plot(test_sub["year"], test_sub["pred_electricity_access"], "s--",
                     color="coral", label="Predicted (test 2021–2023)", linewidth=2)
    if len(fore_sub) > 0:
        axes[0].plot(fore_sub["year"], fore_sub["pred_electricity_access"], "^--",
                     color="purple", label="Forecast (2024–2027)", linewidth=2)
    axes[0].axhline(90, color="green", linestyle="--", linewidth=1, alpha=0.7)
    axes[0].axhline(50, color="red",   linestyle="--", linewidth=1, alpha=0.7)
    axes[0].set_xlabel("Year"); axes[0].set_ylabel("Electricity Access (%)")
    axes[0].set_title("Historical + Predictions + Forecast")
    axes[0].legend(fontsize=8); axes[0].set_ylim(0, 105); axes[0].grid(True, alpha=0.3)

    # Right: GDP trend
    axes[1].plot(sub["year"], sub["gdp_per_capita"] / 1000, "o-", color="#388E3C", linewidth=2)
    axes[1].set_xlabel("Year"); axes[1].set_ylabel("GDP per Capita (USD thousands)")
    axes[1].set_title("GDP per Capita Trend"); axes[1].grid(True, alpha=0.3)
...
def page_widget_b(df):
    ...
    sc = ax.scatter(
        ...
        cmap="RdYlGn", alpha=0.75, s=50, vmin=0, vmax=100,
    )
    plt.colorbar(sc, ax=ax, label="Renewable Energy %")
    ax.axhline(90, color="green", linestyle="--", linewidth=1.5, alpha=0.7, label="90% Minimal")
    ax.axhline(50, color="red",   linestyle="--", linewidth=1.5, alpha=0.7, label="50% Severe")
    ax.set_xlabel("Log₁₀ GDP per Capita (USD)"); ax.set_ylabel("Electricity Access (%)")
    ax.set_title(
...
def page_widget_c(df):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(f"Risk Category: {start_yr} → {end_yr}", fontsize=12, fontweight="bold")

    # Left: transition heatmap (exact Widget C logic from notebook)
    ...
    # Right: who improved and who is stuck
...
**Author:** npydipati | **University:** University of Florida | **Course:** CAP5771 — Data Science
...
def main():
    ...
    # If any output CSV is missing, train fallback models
